...
import { __spreadProps, __spreadValues } from './chunk-PJVWDKLX.js';
...
var removeUndefinedProperties = (t2, n2) =>
  each(assignDeep({}, t2), (t3, n3, o2) => {
    if (t3 === void 0) {
      delete o2[n3];
...
var getTrasformTranslateValue = (t2, n2) =>
  `translate${isObject(t2) ? `(${t2.x},${t2.y})` : `${n2 ? 'X' : 'Y'}(${t2})`}`;
var elementHasDimensions = (t2) => !!(t2.offsetWidth || t2.offsetHeight || t2.getClientRects().length);
var I = {
  w: 0,
  h: 0,
};
var getElmWidthHeightProperty = (t2, n2) =>
  n2
    ? {
        w: n2[`${t2}Width`],
        h: n2[`${t2}Height`],
...
var registerPluginModuleInstances = (t2, n2, o2) =>
  keys(t2).map((s2) => {
...
var $t = (() => ({
  [Ct]: {
    static: () => (t2, n2, o2) => {
      ...
      const e2 = 'scroll';
      const c2 = createDOM(
        `<div class="${ct}" dir="ltr"><div class="${ct}"><div class="${rt}"></div></div><div class="${ct}"><div class="${rt}" style="width: 200%; height: 200%"></div></div></div>`
      );
      const r2 = c2[0];
...
var zt = (() => ({
  [Et]: {
    static: () => (t2, n2, o2, s2) => {
      ...
      const linearPressAnimation = (o3, s3) =>
        animateNumber(o3, n2 - f2, r2 * s3, (o4, s4, e3) => {
          t2(o4);
          if (e3) {
            c2 = easedEndPressAnimation(o4, n2);
          }
        });
      const d2 = animateNumber(
        0,
        f2,
        i2,
        (r3, i3, a3) => {
...
var getPropByPath = (t2, n2) =>
  t2 ? `${n2}`.split('.').reduce((t3, n3) => (t3 && hasOwnProperty(t3, n3) ? t3[n3] : void 0), t2) : void 0;
var It = [0, 33];
var At = [33, 99];
var Tt = [222, 666, true];
var Dt = {
...
var createDOMObserver = (t2, n2, o2, s2) => {
  ...
  const observerCallback = (e3, c3) => {
    if (!isEmptyArray(c3)) {
      ...
      d2((t3) =>
        deduplicateArray(f2).reduce((n3, o3) => {
          push(n3, find(t3, o3));
...
var createObserversSetup = (t2, n2, o2, s2) => {
  ...
  const createDebouncedObservesUpdate = () => {
    ...
    const e3 = debounce(s2, {
      ...
      m(t4, n4) {
        ...
        const [s3] = n4;
        return [
          concat(keys(o4), keys(s3)).reduce((t5, n5) => {
            t5[n5] = o4[n5] || s3[n5];
            return t5;
          }, {}),
        ];
  ...
  const M2 =
    !S2 &&
    createSizeObserver(h2, onSizeChanged, {
      ft: true,
    });
...
var createScrollbarsSetupElements = (t2, n2, o2, s2) => {
  ...
  const initScrollTimeline = (t3) => {
    if (p) {
      ...
      const _setScrollPercentAnimation = (c3) => {
        ...
        const l3 = t3 === 'x';
        const a3 = [
          getTrasformTranslateValue(0, l3),
          getTrasformTranslateValue(`calc(-100% + 100cq${l3 ? 'w' : 'h'})`, l3),
        ];
  ...
  const refreshScrollbarsHandleLength = () => {
    ...
    const createScrollbarStyleFn = (t4) => (n3) => [
      n3.Lt,
      {
  ...
  const refreshScrollbarsHandleOffset = () => {
    if (!p) {
      ...
      const createScrollbarStyleFn = (t4) => (n4) => [
        n4.Lt,
        {
  ...
  const generateScrollbarDOM = (t3) => {
    ...
    push(t3 ? S2 : m2, i3);
    push(y2, [
      appendChildren(e3, c3),
      appendChildren(c3, r3),
      bind(removeElements, e3),
      l3 && l3.kt(i3),
      s2(i3, scrollbarsAddRemoveClass, t3),
    ]);
    return i3;
  };
...
var createScrollbarsSetupEvents = (t2, n2, o2, s2) => (r2, i2, l2) => {
  ...
  const createInteractiveScrollEvents = () => {
    ...
    return addEventListener(h2, 'pointerdown', (o3) => {
      ...
      if (x2) {
        ...
        const M2 = v2();
        const k2 = [
          addEventListener(p2, n3, releasePointerCapture),
          addEventListener(p2, 'selectstart', (t4) => preventDefault(t4), {
            I: false,
          }),
          addEventListener(h2, n3, releasePointerCapture),
          D2 && addEventListener(h2, 'pointermove', (t4) => C3(T2 + (t4[s3] - $3))),
          D2 &&
            (() => {
  ...
  return bind(runEachAndClear, [
    addEventListener(b2, 'pointermove pointerleave', s2),
    addEventListener(g2, 'pointerenter', () => {
      i2(ht, true);
    }),
...
var createScrollbarsSetup = (t2, n2, o2, s2, e2, c2) => {
  ...
  const [m2, O2] = createScrollbarsSetupElements(
    t2,
    e2,
    s2,
    createScrollbarsSetupEvents(
      ...
      e2,
      s2,
      (t3) => isHoverablePointerType(t3) && manageScrollbarsAutoHideInstantInteraction()
    )
  );
  ...
  const M2 = [
    ...
    v2,
    () => f2(),
    addEventListener(C2, 'pointerover', onHostMouseEnter, {
      T: true,
    }),
    addEventListener(C2, 'pointerenter', onHostMouseEnter),
    addEventListener(C2, 'pointerleave', (t3) => {
      if (isHoverablePointerType(t3)) {
        r2 = false;
        if (l2) {
          manageScrollbarsAutoHide(false);
  ...
  return [
    () => bind(runEachAndClear, push(M2, O2())),
    ({ zt: t3, At: n3, Qt: e3, Zt: c3 }) => {
...
var createStructureSetupElements = (t2) => {
  ...
  const appendElements = () => {
    ...
    const prepareWrapUnwrapFocus = (t4) =>
      addEventListener(t4, 'focusin focusout focus blur', stopAndPrevent, {
        A: true,
        I: false,
      });
    const o3 = 'tabindex';
...
var createOverflowUpdateSegment = (t2, s2) => {
  ...
  const setViewportOverflowStyle = (t3) => {
    const createAllOverflowStyleClassNames = (t4) =>
      [x, H, E].map((n3) => createViewportOverflowStyleClassName(overflowCssValueToOverflowStyle(n3), t4));
    const n2 = createAllOverflowStyleClassNames(true).concat(createAllOverflowStyleClassNames()).join(' ');
    d2(n2);
    d2(
      keys(t3)
        .map((n3) => createViewportOverflowStyleClassName(t3[n3], n3 === 'x'))
        .join(' '),
      true
    );
  };
...
var OverlayScrollbars = (t2, n2, o2) => {
  ...
  if (n2 && !r2) {
    ...
    const [g2, h2, b2, w2, y2] = createSetups(
      t2,
      a2,
      () => r3,
      ({ dn: t3, At: n3 }, { Qt: o3, Zt: s3 }) => {
...
// node_modules/ng-scrollable/fesm2022/ng-scrollable.mjs
var _c0 = ['content'];
var _c1 = ['*'];
var createDefer = () => {
...
var OverlayScrollbarsDirective = class _OverlayScrollbarsDirective {
  instanceRef = null;
  requestDefer;
  cancelDefer;
  ...
  osInitialize(target) {
    const init = () => {
      this.instanceRef = OverlayScrollbars(target, this.options() || {}, this.events() || {});
    };
    if (this.defer()) {
  ...
  static ɵfac = function OverlayScrollbarsDirective_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _OverlayScrollbarsDirective)();
  };
  static ɵdir = ɵɵdefineDirective({
    type: _OverlayScrollbarsDirective,
...
var mergeEventListeners = (emits, events) =>
  Object.keys(emits).reduce((obj, name) => {
...
var OverlayScrollbarsComponent = class _OverlayScrollbarsComponent {
  // Signal-based inputs
  options = input(
    ...(ngDevMode
      ? [
  ...
  osInitialized = output();
  osUpdated = output();
  osDestroyed = output();
  osScroll = output();
  contentRef = viewChild(
    'content',
    ...(ngDevMode
      ? [
          {
            debugName: 'contentRef',
          },
        ]
      : [])
  );
  osDirective = viewChild(
    'content',
    ...(ngDevMode
      ? [
          {
            debugName: 'osDirective',
  ...
  static ɵfac = function OverlayScrollbarsComponent_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _OverlayScrollbarsComponent)();
  };
  static ɵcmp = ɵɵdefineComponent({
    type: _OverlayScrollbarsComponent,
    selectors: [['ng-scrollable'], ['', 'ng-scrollable', '']],
    viewQuery: function OverlayScrollbarsComponent_Query(rf, ctx) {
      if (rf & 1) {
        ɵɵviewQuerySignal(ctx.contentRef, _c0, 5);
        ɵɵviewQuerySignal(ctx.osDirective, _c0, 5, OverlayScrollbarsDirective);
      }
      if (rf & 2) {
        ɵɵqueryAdvance(2);
      }
    ...
    consts: [
      ['content', ''],
      ['overlayScrollbars', '', 'data-overlayscrollbars-contents', '', 3, 'options', 'events', 'defer'],
    ],
...
var _instance = null;
var provideWindowScroll = (options = {}, eventListeners) => {
...
export { OverlayScrollbarsComponent, OverlayScrollbarsDirective, provideWindowScroll };
//# sourceMappingURL=ng-scrollable.js.map
