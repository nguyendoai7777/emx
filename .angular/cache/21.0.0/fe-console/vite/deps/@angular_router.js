import { Title } from './chunk-6BERZCM5.js';
import './chunk-O7LISFRQ.js';
import {
...
var Navigation = class {
  ...
  constructor(isAbsolute, numberOfDoubleDots, commands) {
    ...
    if (isAbsolute && commands.length > 0 && isMatrixParams(commands[0])) {
      throw new RuntimeError(
        4003,
        (typeof ngDevMode === 'undefined' || ngDevMode) && 'Root segment cannot have matrix parameters'
      );
    }
  ...
  if (
    ...
    inherited = {
      ...
      resolve: __spreadValues(
        __spreadValues(__spreadValues(__spreadValues({}, route.data), parent.data), routeConfig?.data),
        route._resolvedData
      ),
    };
  } else {
...
var RouterOutlet = class _RouterOutlet {
  ...
  activateWith(activatedRoute, environmentInjector) {
    if (this.isActivated) {
      throw new RuntimeError(
        4013,
        (typeof ngDevMode === 'undefined' || ngDevMode) && 'Cannot activate an already activated outlet'
      );
    }
    this._activatedRoute = activatedRoute;
...
var ApplyRedirects$1 = class ApplyRedirects {
  ...
  createSegments(redirectTo, redirectToSegments, actualSegments, posParams) {
    return redirectToSegments.map((s) =>
      s.path[0] === ':' ? this.findPosParam(redirectTo, s, posParams) : this.findOrReturn(s, actualSegments)
    );
  }
...
function matchWithChecks(segmentGroup, route, segments, injector, urlSerializer, abortSignal) {
  ...
  return runCanMatchGuards(injector, route, segments, urlSerializer, abortSignal).pipe(
    map((v) => (v === true ? result : __spreadValues({}, noMatch$1)))
  );
}
function match(segmentGroup, route, segments) {
  ...
  const parameters =
    res.consumed.length > 0
      ? __spreadValues(__spreadValues({}, posParams), res.consumed[res.consumed.length - 1].parameters)
      : posParams;
  return {
    matched: true,
...
function split(segmentGroup, consumedSegments, slicedSegments, config) {
  if (slicedSegments.length > 0 && containsEmptyPathMatchesWithNamedOutlets(segmentGroup, slicedSegments, config)) {
    const s2 = new UrlSegmentGroup(
      consumedSegments,
      createChildrenForEmptyPaths(config, new UrlSegmentGroup(slicedSegments, segmentGroup.children))
    );
    return {
      segmentGroup: s2,
      slicedSegments: [],
...
var Recognizer$1 = class Recognizer {
  ...
  async expandSegmentAgainstRouteUsingRedirect(injector, segmentGroup, routes, route, segments, outlet, parentRoute) {
    const { matched, parameters, consumedSegments, positionalParamSegments, remainingSegments } = match(
      segmentGroup,
      route,
      segments
    );
...
var CREATE_VIEW_TRANSITION = new InjectionToken(
  typeof ngDevMode !== void 0 && ngDevMode ? 'view transition helper' : ''
);
var VIEW_TRANSITION_OPTIONS = new InjectionToken(
  typeof ngDevMode !== void 0 && ngDevMode ? 'view transition options' : ''
);
...
var NavigationTransitions = class _NavigationTransitions {
  ...
  isUpdatedBrowserUrl() {
    ...
    return (
      currentBrowserUrl.toString() !== targetBrowserUrl?.toString() && !currentNavigation?.extras.skipLocationChange
    );
  }
...
function validateCommands(commands) {
  for (let i = 0; i < commands.length; i++) {
    ...
    if (cmd == null) {
      throw new RuntimeError(
        4008,
        (typeof ngDevMode === 'undefined' || ngDevMode) && `The requested path contains ${cmd} segment at index ${i}`
      );
    }
  }
}
...
var RouterLink = class _RouterLink {
  ...
  constructor(router, route, tabIndexAttribute, renderer, el, locationStrategy) {
    ...
    this.isAnchorElement =
      tagName === 'a' ||
      tagName === 'area' ||
      !!(typeof customElements === 'object' && customElements.get(tagName)?.observedAttributes?.includes?.('href'));
    ...
    this.zone.runOutsideAngular(async () => {
      ...
      this.zone.run(() => {
        this.transitions.events.next(
          new Scroll(routerEvent, this.lastSource === 'popstate' ? this.store[this.restoredId] : null, anchor)
        );
      });
    });
  }
  ...
  static ɵfac = /* @__PURE__ */ (() => {
    ...
    return function NavigationStateManager_Factory(__ngFactoryType__) {
      return (
        ...
        (ɵNavigationStateManager_BaseFactory = ɵɵgetInheritedFactory(_NavigationStateManager))
