...
function throwProviderNotFoundError(token, injectorName) {
  const errorMessage =
    ngDevMode && `No provider for ${stringifyForError(token)} found${injectorName ? ` in ${injectorName}` : ''}`;
...
var ENVIRONMENT_INITIALIZER = new InjectionToken(
  typeof ngDevMode !== void 0 && ngDevMode ? 'ENVIRONMENT_INITIALIZER' : ''
);
...
function getNgModuleDefOrThrow(type) {
  ...
  if (!ngModuleDef) {
    throw new RuntimeError(
      915,
      (typeof ngDevMode === 'undefined' || ngDevMode) && `Type ${stringify(type)} does not have 'ɵmod' property.`
    );
  }
  return ngModuleDef;
}
...
function getDirectiveDefOrThrow(type) {
  ...
  if (!def) {
    throw new RuntimeError(
      916,
      (typeof ngDevMode === 'undefined' || ngDevMode) && `Type ${stringify(type)} does not have 'ɵdir' property.`
    );
  }
  return def;
}
...
function getUndecoratedInjectableFactory(token) {
  ...
  if (paramLength > 0) {
    throw new RuntimeError(
      204,
      ngDevMode && `Can't resolve all parameters for ${stringify(token)}: (${newArray(paramLength, '?').join(', ')}).`
    );
  }
...
function providerToFactory(provider, ngModuleType, providers) {
  ...
  if (isTypeProvider(provider)) {
    ...
    if (isValueProvider(provider)) {
      ...
      factory = (_, flags) =>
        ɵɵinject(resolveForwardRef(provider.useExisting), flags !== void 0 && flags & 8 ? 8 : void 0);
    } else {
      const classRef = resolveForwardRef(provider && (provider.useClass || provider.provide));
  ...
  factory: () => {
    ...
    const errorListener = (e) => {
      if (e.error) {
        ...
        errorHandler(
          new Error(
            ngDevMode ? `An ErrorEvent with no error occurred. See Error.cause for details: ${e.message}` : e.message,
            {
              cause: e,
            }
          )
...
var ReflectionCapabilities = class {
  ...
  _ownParameters(type, parentCtor) {
    ...
    const paramTypes =
      this._reflect && this._reflect.getOwnMetadata && this._reflect.getOwnMetadata('design:paramtypes', type);
...
function registerPostOrderHooks(tView, tNode) {
  ...
  for (let i = tNode.directiveStart, end = tNode.directiveEnd; i < end; i++) {
    ...
    const { ngAfterContentInit, ngAfterContentChecked, ngAfterViewInit, ngAfterViewChecked, ngOnDestroy } =
      lifecycleHooks;
...
function incrementInitPhaseFlags(lView, initPhase) {
  ngDevMode &&
    assertNotEqual(initPhase, 3, 'Init hooks phase should not be incremented after all init hooks have been run.');
...
function assertTNodeType(tNode, expectedTypes, message) {
  ...
  if ((tNode.type & expectedTypes) === 0) {
    throwError(
      message || `Expected [${toTNodeTypeAsString(expectedTypes)}] but got ${toTNodeTypeAsString(tNode.type)}.`
    );
  }
}
function assertPureTNodeType(type) {
  if (
    !(type === 2 || type === 1 || type === 4 || type === 8 || type === 32 || type === 16 || type === 64 || type === 128)
  ) {
...
function getLContext(target) {
  ...
  if (mpValue) {
    if (isLView(mpValue)) {
      ...
      const context =
        existingCtx && !Array.isArray(existingCtx) ? existingCtx : createLContext(lView, nodeIndex, native);
...
var JSACTION_BLOCK_ELEMENT_MAP = new InjectionToken(
  typeof ngDevMode !== void 0 && ngDevMode ? 'JSACTION_BLOCK_ELEMENT_MAP' : '',
  {
...
var JSACTION_EVENT_CONTRACT = new InjectionToken(
  typeof ngDevMode !== void 0 && ngDevMode ? 'EVENT_CONTRACT_DETAILS' : '',
  {
    factory: () => ({}),
  }
);
...
var DEHYDRATED_BLOCK_REGISTRY = new InjectionToken(
  typeof ngDevMode !== void 0 && ngDevMode ? 'DEHYDRATED_BLOCK_REGISTRY' : ''
);
...
function getUrlSanitizer(tag, prop) {
  if (
    (prop === 'src' &&
      (tag === 'embed' || tag === 'frame' || tag === 'iframe' || tag === 'media' || tag === 'script')) ||
    ...
    if (mode & 4) {
      ...
      if (selectorAttrValue !== '') {
        ...
        if (attrIndexInNode > nameOnlyMarkerIdx) {
          ...
          ngDevMode &&
            assertNotEqual(nodeAttrs[attrIndexInNode], 0, 'We do not match directives on namespaced attributes');
...
var UseExhaustiveCheckNoChanges = new InjectionToken(
  typeof ngDevMode !== void 0 && ngDevMode ? 'exhaustive checkNoChanges' : ''
);
...
var _NullComponentFactoryResolver = class {
  resolveComponentFactory(component) {
    throw new RuntimeError(
      917,
      typeof ngDevMode !== 'undefined' && ngDevMode && `No component factory found for ${stringify(component)}.`
    );
  }
};
...
function createRootViewInjector(componentDef, environmentInjector, injector) {
  let realEnvironmentInjector =
    environmentInjector instanceof EnvironmentInjector ? environmentInjector : environmentInjector?.injector;
...
function populateDehydratedViewsInLContainerImpl(lContainer, tNode, hostLView) {
  ...
  const isNodeCreationMode =
    !hydrationInfo || isInSkipHydrationBlock2(tNode) || isDisconnectedNode$1(hydrationInfo, noOffsetIndex);
...
function findMatchingDehydratedViewForDeferBlock(lContainer, lDetails) {
  const dehydratedViewIx =
    lContainer[DEHYDRATED_VIEWS]?.findIndex((view) => view.data[DEFER_BLOCK_STATE$1] === lDetails[DEFER_BLOCK_STATE]) ??
    -1;
    ...
    if (!visitedContainers.has(container)) {
      for (const prov of providerToPath.keys()) {
        ...
        deepForEach(containerDef.imports, (moduleImport) => {
          ...
          isNextStepInPath =
            moduleImport.ngModule === lastContainerAddedToPath || moduleImport === lastContainerAddedToPath;
          if (isNextStepInPath) {
...
async function triggerHydrationFromBlockName(injector, blockName, replayQueuedEventsFn) {
  ...
  if (dehydratedBlockRegistry.has(topmostParentBlock)) {
    ...
    dehydratedBlockRegistry.awaitParentBlock(
      topmostParentBlock,
      async () => await triggerHydrationForBlockQueue(injector, hydrationQueue, replayQueuedEventsFn)
    );
  }
}
...
function cleanupParentContainer(currentBlockIdx, hydrationQueue, dehydratedBlockRegistry) {
  ...
  const parentDeferBlock =
    parentDeferBlockIdx > -1 ? dehydratedBlockRegistry.get(hydrationQueue[parentDeferBlockIdx]) : null;
  if (parentDeferBlock) {
  ...
  if (bindingUpdated(lView, bindingIndex, rawValue)) {
    ...
    try {
      ...
      if (value === false && renderedState === DeferBlockInternalState.Initial) {
        ...
        value === true &&
        (renderedState === DeferBlockInternalState.Initial || renderedState === DeferBlockState.Placeholder)
      ) {
...
function isLongestAnimation(event, nativeElement) {
  ...
  return (
    ...
    ((longestAnimation.animationName !== void 0 && event.animationName === longestAnimation.animationName) ||
      (longestAnimation.propertyName !== void 0 && event.propertyName === longestAnimation.propertyName))
  );
}
...
function initializeControlFirstCreatePass(tView, tNode, lView) {
  ...
  throw new RuntimeError(
    318,
    `'<${tagName}>' is an invalid [field] directive host. The host must be a native form control (such as <input>', '<select>', or '<textarea>') or a custom form control with a 'value' or 'checked' model.`
  );
}
...
function listenToCustomControl(lView, tNode, control, modelName) {
  ...
  listenToOutput(
    ...
    outputName,
    outputName,
    wrapListener(tNode, lView, (value) => control.state().setControlValue(value))
  );
...
function ɵɵconditional(matchingTemplateIndex, contextValue) {
  ...
  const prevContainer =
    prevMatchingTemplateIndex !== -1 ? getLContainer(hostLView, HEADER_OFFSET + prevMatchingTemplateIndex) : void 0;
...
function getLocaleData(normalizedLocale) {
  if (!(normalizedLocale in LOCALE_DATA)) {
    LOCALE_DATA[normalizedLocale] =
      _global.ng && _global.ng.common && _global.ng.common.locales && _global.ng.common.locales[normalizedLocale];
  }
  ...
  if (Array.isArray(provider)) {
    ...
    if (isTypeProvider(provider) || !provider.multi) {
      ...
      const doesViewProvidersFactoryExist =
        existingViewProvidersFactoryIndex >= 0 && lInjectablesBlueprint[existingViewProvidersFactoryIndex];
...
function ɵɵpureFunction5(slotOffset, pureFn, exp1, exp2, exp3, exp4, exp5, thisArg) {
  ...
  return bindingUpdated(lView, bindingIndex + 4, exp5) || different
    ? updateBinding(
        lView,
        bindingIndex + 5,
        thisArg ? pureFn.call(thisArg, exp1, exp2, exp3, exp4, exp5) : pureFn(exp1, exp2, exp3, exp4, exp5)
      )
...
function verifySemanticsOfNgModuleDef(moduleType, allowDuplicateDeclarationsInRoot, importingModule) {
  ...
  function verifyExportsAreDeclaredOrReExported(type) {
    type = resolveForwardRef(type);
    const kind =
      (getComponentDef(type) && 'component') || (getDirectiveDef(type) && 'directive') || (getPipeDef(type) && 'pipe');
    if (kind) {
  ...
  if (nativeNode instanceof Node) {
    if (!nativeNode.hasOwnProperty(NG_DEBUG_PROPERTY)) {
      nativeNode[NG_DEBUG_PROPERTY] =
        nativeNode.nodeType == Node.ELEMENT_NODE ? new DebugElement(nativeNode) : new DebugNode(nativeNode);
    }
...
var OutputEmitterRef = class {
  ...
  subscribe(callback) {
    if (this.destroyed) {
      throw new RuntimeError(
        953,
        ngDevMode && 'Unexpected subscription to destroyed `OutputRef`. The owning directive/component is destroyed.'
      );
    }
  ...
  emit(value) {
    if (this.destroyed) {
      console.warn(
        formatRuntimeError(
          953,
          ngDevMode && 'Unexpected emit for destroyed `OutputRef`. The owning directive/component is destroyed.'
        )
      );
      return;
    }
...
function shouldPreventDefaultBeforeDispatching(actionElement, eventInfoWrapper) {
  return (
    ...
    (eventInfoWrapper.getEventType() === EventType.CLICK || eventInfoWrapper.getEventType() === EventType.CLICKMOD)
  );
}
...
var PLATFORM_DESTROY_LISTENERS = new InjectionToken(
  typeof ngDevMode !== void 0 && ngDevMode ? 'PlatformDestroyListeners' : ''
);
var ENABLE_ROOT_COMPONENT_BOOTSTRAP = new InjectionToken(
  typeof ngDevMode !== void 0 && ngDevMode ? 'ENABLE_ROOT_COMPONENT_BOOTSTRAP' : ''
);
...
function createPlatformFactory(parentPlatformFactory, name, providers = []) {
  ...
  return (extraProviders = []) => {
    ...
    if (!platform) {
      ...
      platform =
        parentPlatformFactory?.(platformProviders) ?? createPlatform(createPlatformInjector(platformProviders, desc));
    }
...
function appendDisconnectedNodeIndex(ngh, tNodeOrNoOffsetIndex) {
  const noOffsetIndex =
    typeof tNodeOrNoOffsetIndex === 'number' ? tNodeOrNoOffsetIndex : tNodeOrNoOffsetIndex.index - HEADER_OFFSET;
...
//# sourceMappingURL=chunk-WKLQYTUL.js.map
